...
interface CustomerTableProps {
  ...
  onEvaluate?: (customer: CustomerDto) => void
}

const levelConfig: Record<number, { label: string; bg: string; text: string }> = {
  0: { label: '战略客户', bg: '#dcfce7', text: '#047857' },
  1: { label: '重点客户', bg: '#dbeafe', text: '#1d4ed8' },
  2: { label: '普通客户', bg: '#f3f4f6', text: '#374151' },
  3: { label: '风险客户', bg: '#fee2e2', text: '#b91c1c' },
}

function CustomerTable({ customers, contractCounts, onRemove, onEvaluate }: CustomerTableProps) {
  if (!customers.length) {
    return (
      <EmptyState
        title="暂无客户记录"
      ...
      <table className="table table-hover align-middle">
        ...
        <tbody>
          {customers.map((customer) => {
            ...
            const levelInfo = levelConfig[customer.level] ?? levelConfig[2]
            return (
              <tr key={customer.id}>
                <td>
