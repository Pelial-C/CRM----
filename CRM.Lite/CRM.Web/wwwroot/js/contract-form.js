(function () {
    ...
    function createItemRow(values) {
        ...
        row.innerHTML = `
            <input data-field="ProductName" value="${escapeAttribute(values?.productName || '')}" class="form-control" required placeholder="产品或服务名称" />
            <input data-field="Quantity" value="${escapeAttribute(values?.quantity || 1)}" class="form-control" type="number" min="1" required />
            <input data-field="UnitPrice" value="${escapeAttribute(values?.unitPrice || 0)}" class="form-control" type="number" min="0" step="0.01" required />
    ...
    function createPlanRow(values) {
        ...
        row.innerHTML = `
            <input data-field="PlanDate" value="${escapeAttribute(values?.planDate || new Date().toISOString().slice(0, 10))}" class="form-control" type="date" required />
            <input data-field="PlanAmount" value="${escapeAttribute(values?.planAmount || '')}" class="form-control" type="number" min="0.01" step="0.01" required placeholder="0.00" />
            <input data-field="Description" value="${escapeAttribute(values?.description || '')}" class="form-control" placeholder="例如首付款、验收款" />
            <button type="button" class="btn btn-outline-danger" data-remove-row>删除</button>`;
        return row;
    }
