...
public class CustomersApiController : ApiControllerBase
{
    ...
    /// <summary>
    /// 评估客户等级（三维度打分：合同额 + 回款及时性 + 满意度）
    /// </summary>
    [HttpGet("{id:int}/evaluation")]
    public async Task<ActionResult<ApiResponse<CustomerEvaluationDto>>> Evaluate(int id)
    {
        return OkResponse(await _customerAppService.EvaluateAsync(id));
    }
